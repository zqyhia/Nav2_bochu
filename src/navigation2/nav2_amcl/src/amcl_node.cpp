...
// globalLocalizationCallback：AMCL全局定位请求（Global Localization）​​ 的回调函数。
// 当外部调用 /global_localization 服务时，AMCL 会重新初始化粒子滤波器，将所有粒子均匀分布在当前地图的自由空间中，用于解决机器人完全丢失定位时的重定位问题。
void
AmclNode::globalLocalizationCallback(
  const std::shared_ptr<rmw_request_id_t>/*request_header*/, // ROS 2 服务请求头（未使用）
  const std::shared_ptr<std_srvs::srv::Empty::Request>/*req*/, // 空请求（无参数）
  std::shared_ptr<std_srvs::srv::Empty::Response>/*res*/) // 空响应（无返回值）
{
  std::lock_guard<std::recursive_mutex> cfl(mutex_); // 通过互斥锁 mutex_ 保护粒子滤波器的操作，防止多线程竞争

  RCLCPP_INFO(get_logger(), "Initializing with uniform distribution");

  pf_init_model(
    pf_, (pf_init_model_fn_t)AmclNode::uniformPoseGenerator, // 遍历地图所有栅格，筛选出自由空间（非障碍物），并在这些区域中随机生成粒子位姿（x, y, θ）。
...
// initialPoseReceivedSrv：AMCL中处理 ​​初始位姿设置服务请求​​ 的回调函数
// 当外部调用 /set_initial_pose 服务时，AMCL 会根据请求中的初始位姿信息重新初始化粒子滤波器，用于手动指定机器人的初始位置。
void
...
{
  global_loc_srv_ = create_service<std_srvs::srv::Empty>( // 全局重定位服务：强制 AMCL 重新进行全局定位，重置粒子滤波器状态
    "reinitialize_global_localization",
    std::bind(&AmclNode::globalLocalizationCallback, this, _1, _2, _3));

  initial_guess_srv_ = create_service<nav2_msgs::srv::SetInitialPose>( // 初始位姿设置服务：手动设置机器人的初始位姿估计
    "set_initial_pose",
    std::bind(&AmclNode::initialPoseReceivedSrv, this, _1, _2, _3));

  nomotion_update_srv_ = create_service<std_srvs::srv::Empty>( // 无运动更新服务：请求在没有实际运动的情况下更新定位
